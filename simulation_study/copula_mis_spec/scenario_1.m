...
%                                               Scenario 1                                                  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Description: The impact of copula model misspecification on local Kendall's tau estimation.
%
% Scenario 1: The true copula is Clayton copula; the misspecified copulas are Gumbel and Frank copulas.
% NOTE: This program uses Scenario 1 as an example. Users can modify parameters to complete Scenarios 2 and 3.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Procedure:
...
% Step 8. Compare performance metrics between the correctly specified and two misspecified copula models.
%
...
%
% Author: Zaixin Huang
% Date: Completed on September 22, 2025
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%% Basic Settings
% Copula configuration
true_copula_type = 'clayton';   % true copula: Clayton; you can change this parameter in Scenarios 2 and 3
true_copula_par = 7;            % copula parameter
measure_type = 'Kendall';       % dependence measure type

% Two misspecified copula models
mis_spec_copula_type_1 = 'gumbel';     % Gumbel copula; you can change this parameter in Scenarios 2 and 3
mis_spec_copula_type_2 = 'frank';      % Frank copula; you can change this parameter in Scenarios 2 and 3
...
b_se_mis_spec_copula_1 = zeros(mc_rep_times,1);    % For estimator based on the misspecified copula model 1
b_se_mis_spec_copula_2 = zeros(mc_rep_times,1);    % For estimator based on the misspecified copula model 2

% 95% confidence intervals for each simulation
b_conf_interval_95_true_copula = zeros(mc_rep_times,2);         % For true copula model
b_conf_interval_95_mis_spec_copula_1 = zeros(mc_rep_times,2);   % For misspecified copula model 1
b_conf_interval_95_mis_spec_copula_2 = zeros(mc_rep_times,2);   % For misspecified copula model 2
...
mc_mis_spec_copula_1_based_local_tau = zeros(mc_rep_times,1);   % Using misspecified copula model 1
mc_mis_spec_copula_2_based_local_tau = zeros(mc_rep_times,1);   % Using misspecified copula model 2
...
for mc_index = 1:mc_rep_times
    fprintf('Simulation %d is in progress...with %d simulations remaining. This program may take some time.\n', mc_index, mc_rep_times - mc_index);
    ...
    % estimate local Kendall's tau using the misspecified copula model 1
    temp_mis_copula_1_par = fun_copula_estimation(mis_spec_copula_type_1,UV);
    % store local Kendall's tau estimated by the misspecified copula model 1
    if strcmpi(ld_type, 'general')
    ...
    % estimated local Kendall's tau using the misspecified copula model 2
    temp_mis_copula_2_par = fun_copula_estimation(mis_spec_copula_type_2,UV);
    % store local Kendall's tau estimated by the misspecified copula model 2 
    if strcmpi(ld_type, 'general')
    ...
    b_matrix_index = bootstrp(B_times,@(x)x,[1:mc_sample_size]);
    ...
    b_mis_spec_copula_1_par = zeros(B_times,1);              % estimated misspecified copula 1 parameter for each bootstrap sample
    b_mis_spec_copula_1_based_local_tau = zeros(B_times,1);  % estimated local Kendall's tau using the misspecified copula model 1 for each bootstrap sample
    
    b_mis_spec_copula_2_par = zeros(B_times,1);              % estimated misspecified copula 2 parameter for each bootstrap sample
    b_mis_spec_copula_2_based_local_tau = zeros(B_times,1);  % estimated local Kendall's tau using the misspecified copula model 2 for each bootstrap sample
    
    % bootstrap iterations: repeat B_times times
    for b_index = 1:B_times
